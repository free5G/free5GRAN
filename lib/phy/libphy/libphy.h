...
namespace free5GRAN {
    namespace phy {
        namespace signal_processing {
            ...
            void modulation(vector<int> bits, int bit_sequence_length, int modulation_scheme, vector<complex<float>> &output_symbols_vector);
            ...
            void channel_mapper(vector<vector<vector<complex<float>>>> input_channels, int num_channels, int *channel_num_symbol, int *channel_num_sc, int *index_first_symbol_channel, int *index_first_sc_channel, int num_SSB_in_this_frame, int num_PDCCH_in_this_frame, int ifft_size, vector<vector<complex<float>>> &output_channel);
            ...
            void ifft(vector<vector<complex<float>>> freq_domain_frame, int *cp_lengths_one_frame, vector<int> data_symbols, int num_symbols_frame, float scaling_factor, vector<complex<float>> &one_frame);
            void map_pdcch(vector<complex<float>> pdcch_symbols, int CORESET_rb_size, int agg_level, int R, int pci, int slot_number, int symbol_number, vector<vector<complex<float>>> &masked_coreset_grid);
            }
        }
}