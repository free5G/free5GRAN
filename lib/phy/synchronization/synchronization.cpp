...
/*
 * get_pss method: try to synchronize PSS for both short and extended cyclic
 * prefix
 */
...
void free5GRAN::phy::synchronization::get_sss(int& n_id_1,
  ...
  for (int i = 0; i < 64; i++) {
    ...
    frequency_sss[64 - i - 1] = complex<float>(fft_out[fft_size - i - 1][0],
   ...
   */
  // Loop over all possible N_ID_1 values
  for (int i = 0; i < free5GRAN::MAX_N_ID_1; i++) {
    int sss_seq[free5GRAN::SIZE_PSS_SSS_SIGNAL];
    // Generate SSS sequence for given N_ID_1 value
    free5GRAN::utils::sequence_generator::generate_sss_sequence(i, n_id_2,
                                                                sss_seq);
    // Correlate generated sequence with received signal
    correlation_value =
        correlate(frequency_sss, sss_seq, free5GRAN::SIZE_PSS_SSS_SIGNAL);
    float abs_value = abs(correlation_value);
    // Check whether or not the correlation peak
    // is greater than the previous highest peak
...
auto free5GRAN::phy::synchronization::correlate(vector<complex<float>> in1,
                                                int* in2,
                                                int size) -> complex<float> {
  /**
   * \fn correlate
