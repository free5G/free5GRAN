...
void free5GRAN::phy::physical_channel::decode_pdcch(
    vector<complex<float>> pdcch_symbols,
    int* dci_bits,
    int agg_level,
    int* reg_index,
   ...
   */
  free5GRAN::phy::signal_processing::hard_demodulation(
      pdcch_symbols, pdcch_bits, agg_level * 6 * 9, 1);

  int c_seq[agg_level * 6 * 9 * 2];
  free5GRAN::utils::sequence_generator::generate_c_sequence(
      (long)pdcch_DMRS_ScrambingID % (long)pow(2, 31), agg_level * 6 * 9 * 2,
      c_seq, 0);

  /*
   * De-scramble pbch_bits to scrambled_bits
   */
  free5GRAN::utils::common_utils::scramble(pdcch_bits, c_seq, dci_bits,
                                           agg_level * 6 * 9 * 2, 0);
}

void free5GRAN::phy::physical_channel::decode_pdsch(
    vector<complex<float>> pdsch_samples,
   ...
   * \partial_imp No layer mapping, antenna port mapping and VRB to PRB
   * de-interleaving \details Details:
   * - Demodulation
   * - Scrambling
   *
   ...
   * Soft demodulation
   */
  free5GRAN::phy::signal_processing::soft_demodulation(pdsch_samples, soft_bits,
                                                       pdsch_samples.size(), 1);
   ...
   */
  free5GRAN::utils::sequence_generator::generate_c_sequence(
      c_init, ds_sch_bits_length, c_seq, 1);
  free5GRAN::utils::common_utils::scramble(
      soft_bits, c_seq, unscrambled_soft_bits, ds_sch_bits_length, 0);
}

void free5GRAN::phy::physical_channel::decode_pbch(
   ...
   */
  int pbch_bits[free5GRAN::SIZE_SSB_PBCH_SAMPLES * 2];
  /*
   * Demodulate PBCH Signal
   */
  free5GRAN::phy::signal_processing::hard_demodulation(
      pbch_symbols, pbch_bits, free5GRAN::SIZE_SSB_PBCH_SAMPLES, 1);

  // Generate de-scrambling sequence
  int c_seq[free5GRAN::SIZE_SSB_PBCH_SAMPLES * 2 * (1 + i_ssb)];
  free5GRAN::utils::sequence_generator::generate_c_sequence(
      pci, free5GRAN::SIZE_SSB_PBCH_SAMPLES * 2 * (1 + i_ssb), c_seq, 0);

  /*
   * De-scramble pbch_bits to scrambled_bits
   */
  free5GRAN::utils::common_utils::scramble(
      pbch_bits, c_seq, bch_bits, free5GRAN::SIZE_SSB_PBCH_SAMPLES * 2,
      i_ssb * free5GRAN::SIZE_SSB_PBCH_SAMPLES * 2);
}

void free5GRAN::phy::physical_channel::compute_pbch_indexes(
  ...
  for (int symbol = 1; symbol < free5GRAN::NUM_SYMBOLS_SSB; symbol++) {
    ...
    for (int i = 0; i < free5GRAN::NUM_SC_SSB; i++) {
      ...
      if (symbol == 1 || symbol == 3) {
        ...
        if (pci % 4 != i % 4 && (i < free5GRAN::INTERVAL_SSB_NO_PBCH_DMRS[0] ||
                                 i > free5GRAN::INTERVAL_SSB_NO_PBCH_DMRS[1])) {
          ref[0][symbol - 1][i] = 1;
          ref[1][symbol - 1][i] = 0;
        } else if (i < free5GRAN::INTERVAL_SSB_NO_PBCH_DMRS[0] ||
                   i > free5GRAN::INTERVAL_SSB_NO_PBCH_DMRS[1]) {
          ref[0][symbol - 1][i] = 0;
          ref[1][symbol - 1][i] = 1;
        } else {
...
void free5GRAN::phy::physical_channel::compute_pdcch_indexes(
    vector<vector<vector<int>>>& ref,
    int n_symb_coreset,
    int agg_level,
    int* reg_bundles,
   ...
   */
  for (int symb = 0; symb < n_symb_coreset; symb++) {
    for (int k = 0; k < agg_level; k++) {
      for (int reg = 0; reg < free5GRAN::NUMBER_REG_PER_CCE; reg++) {
        if (reg % n_symb_coreset == symb) {
          for (int s = 0; s < 12; s++) {
            if (s % 4 == 1) {
              ref[1][symb][reg_bundles[k] * 12 * height_reg_rb +
                           (reg / n_symb_coreset) * 12 + s] = 1;
              ref[0][symb][reg_bundles[k] * 12 * height_reg_rb +
                           (reg / n_symb_coreset) * 12 + s] = 0;
            } else {
              ref[1][symb][reg_bundles[k] * 12 * height_reg_rb +
                           (reg / n_symb_coreset) * 12 + s] = 0;
              ref[0][symb][reg_bundles[k] * 12 * height_reg_rb +
...
void free5GRAN::phy::physical_channel::compute_pdsch_indexes(
    vector<vector<vector<int>>>& ref,
