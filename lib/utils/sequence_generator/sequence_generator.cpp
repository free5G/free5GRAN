...
void free5GRAN::utils::sequence_generator::generate_pss_sequence(
    int n_id_2,
    int* output_sequence) {
  /**
...
void free5GRAN::utils::sequence_generator::generate_sss_sequence(
    int n_id_1,
    int n_id_2,
    int* output_sequence) {
  /**
  ...
  for (int n = 0; n < free5GRAN::SIZE_PSS_SSS_SIGNAL; n++) {
    output_sequence[n] =
        (1 - 2 * x0_seq[(n + m0) % free5GRAN::SIZE_PSS_SSS_SIGNAL]) *
        (1 - 2 * x1_seq[(n + m1) % free5GRAN::SIZE_PSS_SSS_SIGNAL]);
  }
}

void free5GRAN::utils::sequence_generator::generate_pbch_dmrs_sequence(
    int pci,
    int i_bar_ssb,
   ...
   * \param[out] output_sequence: output sequence
   */
  int c_init = pow(2, 11) * (i_bar_ssb + 1) * (pci / 4 + 1) +
               pow(2, 6) * (i_bar_ssb + 1) + (pci % 4);
  int seq[2 * free5GRAN::SIZE_SSB_DMRS_SAMPLES];
  generate_c_sequence(c_init, 2 * free5GRAN::SIZE_SSB_DMRS_SAMPLES, seq, 0);
  for (int m = 0; m < free5GRAN::SIZE_SSB_DMRS_SAMPLES; m++) {
    output_sequence[m] =
        (float)(1 / sqrt(2)) *
...
void free5GRAN::utils::sequence_generator::generate_c_sequence(
    long c_init,
    int length,
    int* output_sequence,
    int demod_type) {
...
void free5GRAN::utils::sequence_generator::generate_pdcch_dmrs_sequence(
    int nid,
    int slot_number,
    int symbol_number,
    complex<float>* output_sequence,
   ...
   */
  long c_init = (long)(pow(2, 17) * (14 * slot_number + symbol_number + 1) *
                           (2 * nid + 1) +
                       2 * nid) %
                (long)pow(2, 31);
  int seq[2 * size];
...
void free5GRAN::utils::sequence_generator::generate_pdsch_dmrs_sequence(
    int n_symb_slot,
    int slot_number,
    int symbol_number,
    int n_scid,
